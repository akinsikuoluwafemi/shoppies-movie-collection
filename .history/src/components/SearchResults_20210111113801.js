...
export default function SearchResults() {
  ...
  const { selectedMovie, setSelectedMovie } = useContext(SelectedMovieContext);
    ...
    const onMovieSelect = (movie) => {
      ...
      changeButtonAfterFulfilled(tempObj.Title, tempObj.Year);

    }
  
    const changeButtonAfterFulfilled = (title,year) => {
      let mappedMovies = allMovies.map(movies => movies.Title === title && movies.Year === year)
      console.log(mappedMovies[true])
    }
